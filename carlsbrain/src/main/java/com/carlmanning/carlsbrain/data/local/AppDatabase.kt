...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /**
         * Buckets on journal entries, and a bucket plus a reminder rule on templates.
         *
         * The index IS declared on JournalEntryEntity as well — that pairing is what v2.8 got
         * wrong and crashed on launch for. Room compares the two exactly; an index in one place
         * and not the other fails validation on the first database access.
         */
        val MIGRATION_26_27 = object : Migration(26, 27) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE journal_entries ADD COLUMN bucketId INTEGER")
                db.execSQL("CREATE INDEX IF NOT EXISTS index_journal_entries_bucketId ON journal_entries (bucketId)")
                db.execSQL("ALTER TABLE journal_templates ADD COLUMN bucketId INTEGER")
                db.execSQL("ALTER TABLE journal_templates ADD COLUMN reminderRule TEXT NOT NULL DEFAULT ''")
            }
        }

        /**
         * Adds the loose-thread state table.
         *
         * A new table rather than columns on each source: a thread can point at a to-do, a
         * meeting, a note or a journal draft, so there is nowhere else the dismissal could live
         * without repeating the same two columns four times.
         */
        val MIGRATION_27_28 = object : Migration(27, 28) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    """
                    CREATE TABLE IF NOT EXISTS loose_thread_state (
        ...
        /**
         * Chat threads become syncable.
         *
         * Existing threads default to unsynced, so the first sync after this migration
         * publishes Carl's whole chat history rather than only conversations he touches
         * afterwards. That is the behaviour he asked for — the point of syncing chat is to
         * pick up an existing conversation on the laptop.
         */
        val MIGRATION_28_29 = object : Migration(28, 29) {
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL("ALTER TABLE chat_threads ADD COLUMN isSynced INTEGER NOT NULL DEFAULT 0")
            }
        }
